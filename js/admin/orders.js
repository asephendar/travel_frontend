...
export function displayOrders(orders) {
    ...
    paginatedOrders.forEach((order, index) => {
        // const availabilityText = order.status_payment ? "Yes" : "No";
        const orderRow = document.createElement('tr');
        orderRow.innerHTML = `
            <td class="text-center">${start + index + 1}</td>
            <td class="text-center">${order.order.user.username}</td>
            <td class="text-center">${order.order.user.full_name}</td>
            <td class="text-center">${order.order.user.email}</td>
            <td class="text-center">${order.order.user.phone_number}</td>
            ...
            <td class="text-center"><span class="badge bg-success">${order.order.status_payment}</span></td>
            <td class="text-center">${order.order.total_amount}</td>
            <td class="text-center">
                <button type="button" class="btn btn-warning btn-sm edit-orders-btn" data-bs-toggle="modal" data-bs-target="#editOrdersModal" data-order-id="${order.id_order}">
                <i class="bi bi-pencil-square"></i> Edit
                </button>
                 <button type="button" class="btn btn-danger btn-sm delete-orders-btn" data-order-id="${order.id_order}">
